...
solve_ids_step(C, RC, M, RM, DC, D, MD) :-
    ids_move(C, M, M_, RC_),
    ND is D + 1,
    solve_ids_step(RC_, RC, M_, RM, DC, ND, MD).
...
solve_ids_concurrent(C, RM, DONE_C, D, MD) :-
    D < MD,
    ND is D + 1, % Increase current depth and recursively repeat
    solve_ids_concurrent(C, RM, DONE_C, ND, MD).
